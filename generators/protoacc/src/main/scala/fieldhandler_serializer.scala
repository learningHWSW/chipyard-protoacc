    ...
    is (S_WAIT_CMD) {
      when (io.ops_in.bits.end_of_message === true.B) {
        ...
        ProtoaccLogger.logInfo(logPrefix + " S_WAIT_CMD: accept op: src_data_addr 0x%x, src_data_type %d, is_repeated 0x%x, is_packed 0x%x, field_number %d, wire_type %d, cpp_size_log2 %d, is_varint_signed %d\n",
          WireDefault(io.ops_in.bits.src_data_addr),
          WireDefault(io.ops_in.bits.src_data_type),
          WireDefault(is_repeated),
          WireDefault(is_packed),
          WireDefault(io.ops_in.bits.field_number),
          WireDefault(wire_type),
          WireDefault(cpp_size_log2),
          WireDefault(is_varint_signed)
        )
